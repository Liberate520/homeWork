...
/*
Класс выглядит некрасиво, но он обладает достаточно ограниченным функционалом:
наполнить семейное древо тестовыми данными - экземплярами класса Human.
В коммерческой разработки данный класс не существовал бы в силу наличия тест кейсов.

В данном случае считаю, что данный класс реализует свою задачу, и учитывая его узкую направленность,
он не должен быть декомпозирован. По сути, данный класс - это не отдельный модуль, а командная утилита,
временно вызываемая экземпляром класса Service.

Возможно, было бы правильно вызывать метод данного класса не через Сервис, но думаю, в данном случае
