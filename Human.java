...
public class Human implements Serializable, Comparable<Human> {
    ...
    public String getAllChildren() {
        ...
        if (this.children.size() > 0) {
            ...
            for (Human child : this.children) {
                kids.append(child.getGender().equals("М") ? "Сын" : "Дочь").append(" - ").append(child.getName())
                        .append(" ");
            }
        } else {
    ...
    public String getAllSistersOrBrothers(String gender) {
        ...
        if (!(this.father == null)) {
            ...
            if (sistOrBroth.length() == 0) {
                sistOrBroth.append("У ").append(this.getName())
                        .append(gender.equals("Ж") ? " нет сестёр" : " нет братьев");
            }
        } else
            sistOrBroth.append("У ").append(this.getName()).append("родители неизвестны.");
