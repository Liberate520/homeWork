...
public class FamilyTree {
    ...
    public String getInfo() {
        ...
        return sb.append("Генеалогическое древо человека - ").append(getFullName(person)).append(": \n")
    ...
    public Person getRelativeByFamilyStatus(Person person, FamilyStatus status){
        return person.getRelatives().stream().filter(x -> x.getStatus().equals(status)).findFirst()
                .orElse(new Relative()).getPerson();
    }

    public Set<Person> getRelativesByFamilyStatus(Person person, FamilyStatus status){
        return person.getRelatives().stream().filter(x -> x.getStatus().equals(status))
    ...
    public String getRelativesFullNameByFamilyStatus(Person person, FamilyStatus status){
        ...
        Set<Relative> relatives = person.getRelatives().stream().filter(x -> x.getStatus().equals(status))
    ...
    public Person searchByName(String name) {
        return person.getRelatives().stream().filter(x -> x.getPerson().getFirstName().equalsIgnoreCase(name))
